defmodule UserDocsWeb.ProcessesLiveTest do
  ...
  describe "Index" do
    ...
    test "lists all processes", %{conn: conn, process: process} do
      {:ok, _index_live, html} = live(conn, Routes.processes_index_path(conn, :index))
    ...
    test "saves new process", %{conn: conn} do
      {:ok, index_live, _html} = live(conn, Routes.processes_index_path(conn, :index))

      assert index_live |> element("a", "New Process") |> render_click() =~
               "New Process"

      assert_patch(index_live, Routes.processes_index_path(conn, :new))
      ...
      {:ok, _, html} =
        ...
        |> render_submit()
        |> follow_redirect(conn, Routes.processes_index_path(conn, :index))
    ...
    test "updates process in listing", %{conn: conn, process: process} do
      ...
      assert_patch(index_live, Routes.processes_index_path(conn, :edit, process))
  ...
  describe "Show" do
    ...
    test "displays process", %{conn: conn, process: process} do
      {:ok, _show_live, html} = live(conn, Routes.processes_show_path(conn, :show, process))
    ...
    test "updates process within modal", %{conn: conn, process: process} do
      {:ok, show_live, _html} = live(conn, Routes.processes_show_path(conn, :show, process))

      assert show_live |> element("a", "Edit") |> render_click() =~
               "Edit Process"

      assert_patch(show_live, Routes.processes_show_path(conn, :edit, process))
      ...
      {:ok, _, html} =
        ...
        |> form("#process-form", process: @update_attrs)
        |> render_submit()
        |> follow_redirect(conn, Routes.processes_show_path(conn, :show, process))
