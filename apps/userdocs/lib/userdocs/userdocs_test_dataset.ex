defmodule UserDocs.TestDataset do
  ...
  def delete() do
    ...
    Enum.each(Projects.list_versions, fn(v) -> Projects.delete_version(v) end)
    Enum.each(Projects.list_projects, fn(p) -> Projects.delete_project(p) end)
    Enum.each(Automation.list_step_types, fn(st) -> Automation.delete_step_type(st) end)
